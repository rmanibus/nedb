...
describe("Indexes", () => {
  describe("Insertion", () => {
    ...
    it("Can insert pointers to documents in the index correctly when they have compound fields", () => {
      ...
      // The underlying BST now has 3 nodes which contain the docs where it's expected
      idx.tree.getNumberOfKeys().should.equal(3);
      assert.deepEqual(idx.tree.search({ tf: "hello", tg: "world" }), [
        { a: 5, tf: "hello", tg: "world" },
      ]);
      assert.deepEqual(idx.tree.search({ tf: "hello", tg: "bloup" }), [
        { a: 8, tf: "hello", tg: "bloup" },
      ]);
      assert.deepEqual(idx.tree.search({ tf: "bloup", tg: "bloup" }), [
        { a: 2, tf: "bloup", tg: "bloup" },
      ]);
    ...
    it("Inserting twice for the same compound fieldName in a unique index will result in an error thrown", () => {
    ...
    describe("Compound Indexes", () => {
      it("Supports arrays of fieldNames", () => {
        ...
        // The underlying BST now has 3 nodes which contain the docs where it's expected
        idx.tree.getNumberOfKeys().should.equal(3);
        assert.deepEqual(idx.tree.search({ tf: "hello", tf2: 7 }), [
          { a: 5, tf: "hello", tf2: 7 },
        ]);
        assert.deepEqual(idx.tree.search({ tf: "hello", tf2: 6 }), [
          { a: 8, tf: "hello", tf2: 6 },
        ]);
        assert.deepEqual(idx.tree.search({ tf: "bloup", tf2: 3 }), [
          { a: 2, tf: "bloup", tf2: 3 },
        ]);
