...
export default class Persistence {
  ...
  persistCachedDatabase(cb) {
    ...
    Object.keys(this.db.indexes).forEach((fieldName) => {
      if (fieldName !== "_id") {
        ...
        toPersist += `${self.afterSerialization(
          model.serialize({
            $$indexCreated: {
              fieldName: self.db.indexes[fieldName].fieldName,
  ...
  persistNewState(newDocs, cb) {
    ...
    storage.appendFile(self.filename, toPersist, "utf8", (err) =>
      callback(err)
    );
  }
      ...
      [
        (cb) => {
          Persistence.ensureDirectoryExists(
            ...
            (err) => {
              storage.ensureDatafileIntegrity(self.filename, (err) => {
                storage.readFile(self.filename, "utf8", (err, rawData) => {
                  ...
                  try {
                    const treatedData = self.treatRawData(rawData);
