import { BinarySearchTree } from "@rmanibus/binary-search-tree";
...
export default class Index {
  constructor(options) {
    ...
    const compareFunc = util.isArray(this.fieldName)
      ? model.compoundCompareThings(this.fieldName)
      : model.compareThings;
    this.treeOptions = {
      unique: this.unique,
  ...
  insert(doc) {
    ...
    const key = getDotValues(doc, this.fieldName);
  ...
  getMatching(value) {
    ...
    value.forEach((v) => {
      self.getMatching(v).forEach((doc) => {
        _res[doc._id] = doc;
      });
    });
