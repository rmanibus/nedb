...
function modify(obj, updateQuery) {
  ...
  const firstChars = _.map(keys, (item) => item[0]);
  const dollarFirstChars = _.filter(firstChars, (c) => c === "$");
  let newDoc;
  let modifiers;
...
function matchQueryPart(obj, queryKey, queryValue, treatObjAsValue) {
  ...
  if (
    ...
    keys = Object.keys(queryValue);
    firstChars = _.map(keys, (item) => item[0]);
    dollarFirstChars = _.filter(firstChars, (c) => c === "$");
