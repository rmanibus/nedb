...
export default class Datastore extends EventEmitter {
    ...
    async.waterfall([
      ...
      (cb) => {
        ...
        if (basicQueryKeys.length > 0) {
          return cb(
            null,
            self.indexes[basicQueryKeys[0]].getMatching(
              query[basicQueryKeys[0]]
            )
          );
        }
        ...
        indexNames.forEach((indexName) => {
          ...
          const subIndexNames = indexName.split(",");
          if (
            _.intersection(subIndexNames, usableQueryKeys).length ===
            subIndexNames.length
          ) {
        ...
        if (compoundQueryKeys.length > 0) {
          return cb(
            null,
            self.indexes[compoundQueryKeys[0]].getMatching(
              _.pick(query, compoundQueryKeys[0])
            )
          );
        }
        ...
        Object.keys(query).forEach((k) => {
          if (
            query[k] &&
            Object.prototype.hasOwnProperty.call(query[k], "$in")
          ) {
            usableQueryKeys.push(k);
          }
        });
  ...
  _insert(newDoc, cb) {
    ...
    return this.persistence.persistNewState(
  ...
  _update(query, updateQuery, options, cb) {
    ...
    async.waterfall([
      (cb) => {
        ...
        return cursor.limit(1)._exec((err, docs) => {
          if (err) {
            return callback(err);
          }
          if (docs.length === 1) {
      ...
      () => {
        ...
        self.getCandidates(query, (err, candidates) => {
          ...
          return self.persistence.persistNewState(updatedDocs, (err) => {
            if (err) {
              return callback(err);
            }
